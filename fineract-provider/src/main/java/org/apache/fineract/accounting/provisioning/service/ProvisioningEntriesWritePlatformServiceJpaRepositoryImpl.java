...
public class ProvisioningEntriesWritePlatformServiceJpaRepositoryImpl implements ProvisioningEntriesWritePlatformService {
    ...
    public void generateLoanLossProvisioningAmount() {
        ...
        try {
            ...
            //Provisioning Entry can be created by users and is also auto generated by a Schedular Job
            //We are allowed to have one provisinal entry on a date
            //We can Just log the "detailed messege" that is auto generated on creation of Exception
            LOG.error("{}",peace.getDefaultUserMessage());
        } catch (DataIntegrityViolationException dive) {
            //DataIntegrityViolation is thrown when an attempt to insert or update data results in violation of an integrity constraint.
            //Adding a entry with same primary key
            //I am not sure if this can occur in this case since we are adding the entries
            //It might be related to a to date being a primary key but we are already checking for the date not existing before
            //We should not end up here looking that currently Provisional entries are added peacefully right now
